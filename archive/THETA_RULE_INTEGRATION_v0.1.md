# θ-Rule Engine — Arx Integration v0.1

> **Status:** REVISED DRAFT (Axioma sign-off received)
> **Date:** 2026-07-08
...
3. If still tied → FLAG for human review, do not act
...
1. **AIB model readiness.** The θ-Rule concept depends on AIB being trained and available. **v0.1 fallback:** Sentence-transformers (`all-MiniLM-L6-v2`) for initial compilation. 384-dimensional vectors. Cosine similarity matching with configurable threshold. The interface is identical, so the swap to AIB in v0.2 is transparent.
