# Multi-Agent Multi-Turn Event Loop v0.2.0
...
**Date:** 2026-07-08  
**Based on:** AXIOMA v1.9.1 event loop, Skye meta-cognition (thought-addition v4), Neurogossip-agent-v3 session management, The Agora protocol, Arx architecture v0.2.0  
...
- **Meaningless-message resistant** — content-free acknowledgements, lone emoji, and "ok" are suppressed, but **context-aware affirmations and negations are allowed**
...
| **E5** | **Context-aware affirmations and negations are allowed.** Bare "yes" and "no" responses are **not** unconditionally suppressed. They are allowed when they are direct responses to a pending question. | Prevents breaking collaborative reasoning pipelines where binary answers are valid responses. |
| **E6** | **Every request expects a response.** When an agent sends a request (a message tagged with `request`), the event loop tracks it. If no response arrives within a configurable timeout, the request is flagged as orphaned and escalated. | Prevents dropped work items. |
| **E7** | **Fan-out is tracked as a unit.** A single request to multiple recipients is not complete until all recipients have responded. Partial results are available for inspection but do not satisfy the request. | Ensures multi-agent coordination is reliable. |
| **E8** | **Human-in-the-loop pauses the agent clock.** When a request is sent to a human, the conversation's productivity timer is paused. The conversation resumes when the human responds. | Humans think at a different timescale. |
| **E9** | **Transitive timeout extensions for request trees.** If any child node in a request tree is paused or waiting on a human, the timeout timer of all parent request nodes in the tree is paused automatically. | Prevents parent requests from timing out while waiting for a human response to a child request. |
| **E10** | **Conversations are isolated by topic, not just by peer.** An agent can have multiple concurrent conversations with the same peer on different topics. Each topic has its own productivity score, exchange counter, and request tracker. | Prevents a stalled topic from blocking other work with the same peer. |
| **E11** | **The event loop is a scheduler, not a reasoning engine.** It decides *when* to process an incoming message, but the actual processing (what to say, what to compute) is delegated to the agent's cognition layer. | Separation of concerns. The event loop manages conversation flow; the cognition layer manages content. |
| **E12** | **Disengagement is graceful and explicit.** When the event loop decides to end a conversation, it sends a closing message (configurable per profile) and then stops responding to that topic. The peer is not left wondering if the agent is still listening. | Clear communication prevents confusion. |
| **E13** | **Re-engagement is possible after silence.** If a conversation has been closed (by disengagement or exchange cap), a new message from the peer on a *different* topic starts a fresh conversation. The old conversation is archived, not deleted. | Prevents permanent lockout while maintaining caps. |
| **E14** | **The event loop is observable.** Every decision (engage, disengage, suppress, escalate, archive) is logged to a structured event log. The web UI displays the event loop state for each active conversation. | Debugging and transparency. |
| **E15** | **Productivity scoring uses lightweight heuristics, not LLM calls.** To avoid latency and cost multiplication, productivity scoring components use keyword checks, cosine distance on existing embeddings, and structural progress indicators rather than full LLM calls on every turn. | Prevents 3× LLM cost per message exchange. |
| **E16** | **Productivity scoring prioritizes structural progress over text characteristics.** Text length and semantic distance are capped in the scoring formula. Closed sub-goals and verified steps carry more weight than verbose output. | Prevents agents from gaming the productivity score by outputting long, verbose paragraphs. |
...
flowchart TB
    ...
    subgraph EventLoop["Master Event Loop"]
        ...
        subgraph Scheduler["Scheduler"]
            ...
            ProdScorer["Productivity Scorer<br/>(lightweight heuristics)"]
            ExchangeCounter["Exchange Counter"]
            RequestTracker["Request Tracker<br/>(with transitive timeout)"]
            Timer["Timer / Timeout Manager"]
        end
        
        subgraph Decision["Decision Engine"]
            EngageDecider["Engage / Disengage Decider"]
            SuppressFilter["Meaningless Message Filter<br/>(context-aware affirmations)"]
...
4. Meaningless Message Filter checks if the message is content-free
   ...
   b. If context-aware affirmation/negation (response to pending question) → allow
   c. If meaningful → proceed
...
6. Productivity Scorer updates the conversation's productivity score
   a. Uses lightweight heuristics (keyword checks, cosine distance, structural progress)
   b. If score below threshold → consider disengagement
...
class Request:
    ...
    is_paused: bool                      # True if waiting on a child request that is paused
```
...
class ProductivityScore:
    ...
    # Components (computed via lightweight heuristics)
    information_gain: float              # Cosine distance between new response and recent history
    progress_toward_goal: float          # Structural progress (closed sub-goals, verified steps)
    response_relevance: float            # Keyword match with last request
    engagement_depth: float             # Presence of reasoning markers (capped at 0.3 max)
...
### 5.2 Component Computation (Lightweight Heuristics)

To avoid multiplying LLM costs, all components are computed via **lightweight heuristics**:

**Information gain** is estimated by:
- Cosine distance between the new response's embedding (from a small, fast model like `all-MiniLM-L6-v2`) and the embeddings of the last 3 messages in the conversation history
- Presence of new keywords (theorem names, lemma numbers, data points) not present in the last 3 messages
- Absence of repetition (exact string match with any of the last 3 messages)

**Progress toward goal** is estimated by:
- For audit conversations: number of steps audited / total steps (structural progress)
- For proof conversations: number of sub-goals closed / total sub-goals (structural progress)
- For verification conversations: number of claims verified / total claims (structural progress)
- For general conversations: keyword-based progress detection (e.g., "done", "completed", "finished", "resolved")

**Response relevance** is estimated by:
- Whether the response contains keywords from the last request (keyword overlap)
- Whether the response is on-topic (topic embedding cosine similarity with the conversation's topic embedding)
- Whether the response introduces unrelated tangents (detected by low topic embedding similarity)

**Engagement depth** is estimated by:
- Response length (capped at 0.3 max contribution to prevent length gaming)
- Presence of reasoning markers ("because", "therefore", "since", "implies") — keyword check
- Presence of specific references (theorem names, lemma numbers, data points) — regex check
- Absence of vague language ("maybe", "perhaps", "I'm not sure" without follow-up) — keyword check
...
5. Peer explicitly says "goodbye" or "end conversation" (detected by keyword check)
...
### 7.2 Context-Aware Affirmations and Negations
...
2. **Recent History Check:** If no explicit request ID is attached but the last message in the thread from the peer (within the last 2 turns) contained a question mark (`?`) or was classified as a question by a simple prompt classifier, the simple response is **allowed**.
...
This prevents the critical bug where binary answers to mathematical questions (e.g., "Does this lemma apply here?" → "yes") are silently dropped, breaking collaborative reasoning.

### 7.3 Soft Rules (lightweight heuristic, configurable per profile)

Messages that pass hard rules but are still content-free are judged by lightweight heuristics:

- **Acknowledgement with no new information**: "That's a good point." → suppress if no new keywords
...
### 8.2 Request Tree with Transitive Timeout Extensions
...
**Transitive timeout extensions:**
- If any child node in a request tree is paused (e.g., waiting on a human), the timeout timer of all parent request nodes in the tree is **paused automatically**
- When the child node resumes (human responds), the parent timeout timers resume
- This prevents parent requests from timing out while waiting for a human response to a grandchild request
...
3. Event loop:
   ...
   c. Pauses all parent request timeouts in the request tree
   d. Sends the request to the Web UI
   e. Web UI displays the request with a response form
4. Human responds via Web UI
5. Event loop:
   a. Sets conversation status to "active"
   b. Resumes the productivity timer
   c. Resumes all parent request timeouts
   d. Routes the response back to the agent
   e. Agent continues processing
```
...
### 10.2 Bridge Pattern with Bounded Queues

The master event loop runs in the **agent's main thread** (or async loop). Neurogossip-v3 runs in a **background thread** (or separate async task). Communication between them is via **bounded thread-safe queues** with explicit backpressure:
...
**Queue capacity:** 1000 messages per queue. If the queue is full, the sender blocks (backpressure) or drops low-priority ambient events (Agora broadcasts without @-mention).

**Dropping policy for low-priority events:**
- Agora broadcasts without @-mention: dropped if queue > 80% full
- Agora broadcasts with @-mention: never dropped
- Neurogossip direct messages: never dropped
- Web UI messages: never dropped
...
defaults:
  ...
  queue_capacity: 1000  # Bounded queue size
...
- Implement per-turn productivity score computation (lightweight heuristics)
...
- Implement context-aware affirmation/negation detection
- Implement soft-rule heuristic checks
- Implement suppression behavior
- Profile-dependent configuration
- Unit tests with edge cases (including binary answers to questions)

### Phase 4: Request-Response Matching (Week 4)

- Implement request lifecycle
- Implement fan-out tracking
- Implement request tree propagation with transitive timeout extensions
...
- Implement pause/resume with transitive timeout extensions
...
- Implement bridge pattern with bounded queues
- Implement queue protocol with backpressure
...
| G4 | **Context-aware affirmations** | Binary answers to pending questions are allowed; bare affirmations without context are suppressed | 3 |
| G5 | **Request tracking** | 100% of requests tracked; 100% of responses matched; orphan detection within 1s of timeout | 4 |
| G6 | **Fan-out correctness** | Fan-out with N recipients completes only after all N respond; partial results available | 4 |
| G7 | **Transitive timeout extensions** | Parent request timeout is paused when child request is waiting on human; resumes when human responds | 4 |
| G8 | **Multi-conversation fairness** | No single conversation starves others for > 3 consecutive exchanges | 5 |
| G9 | **Human-in-the-loop** | Human request pauses conversation; human response resumes it; no messages lost during pause | 6 |
| G10 | **Neurogossip-v3 integration** | Messages flow correctly in both directions; archiving/un-archiving works; history is consistent | 7 |
| G11 | **Queue backpressure** | Bounded queue drops low-priority events when > 80% full; never drops high-priority events | 7 |
| G12 | **Agora integration** | @-mentioned messages create conversations; broadcast messages do not; ambient awareness works | 8 |
| G13 | **Event log completeness** | Every event type is logged; log is queryable by conversation_id, peer_id, and time range | 9 |
| G14 | **Long conversation stability** | 1000-turn simulated conversation completes without error; no memory leak | 10 |
| G15 | **Productivity disengagement** | Simulated unproductive conversation is disengaged within 5 turns of threshold breach | 10 |
...
2. **Cross-agent productivity calibration.** If Agent A has a productivity score of 0.6 and Agent B has 0.4, is A really more productive, or are the scales different? Proposal: normalize scores per agent pair based on historical averages.

3. **Request tree depth limit.** How deep can a request tree grow before it becomes unmanageable? Proposal: limit to depth 5 (root → 4 levels of delegation). Beyond that, the agent must consolidate before further delegation.

4. **Human interruption priority.** If a human interrupts an agent-agent conversation, should the agent pause the agent conversation immediately or finish its current turn? Proposal: finish the current turn (to avoid partial reasoning), then switch to human.

5. **Closing message effectiveness.** How do we know the closing message is effective? Proposal: track whether the peer sends a new meaningful message after the closing message. If > 50% do, the closing message is not effective enough.

6. **Multi-profile conversations.** If Arx is in `auditor` profile and a peer asks a question better suited for `explorer`, should the event loop switch profiles? Proposal: flag the mismatch and ask the human or peer to confirm the profile switch.

---

## Appendix A: Comparison with Existing Systems

| Feature | AXIOMA v1.9.1 | Neurogossip-v3 | Skye Meta-Cog | Master Event Loop v0.2.0 |
|---------|---------------|----------------|---------------|--------------------------|
| Per-peer history isolation | ✅ | ✅ (session) | ❌ | ✅ |
| Exchange cap | ✅ (250) | ❌ | ❌ | ✅ (configurable) |
| Meaningless suppression | ✅ (hard rules) | ❌ | ❌ | ✅ (hard + soft + context-aware) |
| Context-aware affirmations | ❌ | ❌ | ❌ | ✅ |
| Request-response matching | ❌ | ✅ | ❌ | ✅ |
| Fan-out tracking | ❌ | ✅ | ❌ | ✅ |
| Transitive timeout extensions | ❌ | ❌ | ❌ | ✅ |
| Productivity scoring (lightweight) | ❌ | ❌ | ✅ (reasoning) | ✅ (conversation) |
...
| **Transitive timeout extension** | Automatic pausing of parent request timeouts when a child request is paused |
...
| **Context-aware affirmation** | A "yes"/"no" response that is allowed because it answers a pending question |
...
| **Backpressure** | Queue-level flow control that drops low-priority events when capacity is exceeded |
