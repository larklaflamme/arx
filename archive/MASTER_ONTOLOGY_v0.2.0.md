# Master Ontology — Design v0.2.0

**Status:** Final (incorporating all independent review feedback)  
**Date:** 2026-07-08  
**Project root:** `/home/ubuntu/arx/`  
**Related documents:**
  - [ARX_ARCHITECTURE_v0.2.0.md](ARX_ARCHITECTURE_v0.2.0.md) — Arx agent architecture (§3.9)
...
1. **LMFDB** — verified computational data on L-functions, modular forms, elliptic curves, number fields, and related objects (confidence 1.0). Contains 3,824,372 elliptic curves over ℚ (in 2,917,287 isogeny classes), 767,518 elliptic curves over number fields (in 367,933 isogeny classes), plus modular forms, L-functions, number fields, and other objects across dozens of categories.
...
The ontology is stored as a **property graph** (SQLite + Qdrant for v0.2.0, Neo4j when needed), with every node and edge carrying **provenance metadata** indicating which source(s) contributed it and at what confidence level. Speculative concepts are partitioned into a separate sub-graph to prevent epistemic pollution during proof auditing.
...
| O10 | **Epistemic isolation is enforced.** | Speculative concepts (confidence < 0.8, non-mathematical domains) are partitioned into a separate sub-graph. Verification queries are restricted to the VERIFY sub-graph. |
...
- **No full mirror.** The LMFDB is too large to mirror in v0.2.0. Cache referenced objects over time.
...
- Agents propose additions via a **sandboxed database table** or dedicated queue, not by pushing directly to the Git repository
...
- Sandboxed queue for additions and modifications
...
3. **Profile-dependent access:**
   * `auditor`, `verifier`, `reviewer` profiles: VERIFY sub-graph only
   * `prover` profile: VERIFY sub-graph only
   * `explorer` profile: Both sub-graphs accessible
...
│  - Sub-graph partitioning (VERIFY / RESEARCH)     │
└─────────────────────────────────────────────────┘
```
...
| LMFDB vs any other source | LMFDB wins (verified data). **Log the conflict** even when LMFDB wins, so operators can inspect mismatches in concept alignment. |
...
### 5.1 Primary Store: SQLite (v0.2.0)

**Rationale:** SQLite handles graph queries via recursive CTEs for the scale we expect in v0.2.0 (hundreds to low thousands of nodes). Properly indexed SQLite tables can handle 100,000+ nodes with recursive CTEs. No additional infrastructure to maintain.
...
CREATE TABLE nodes (
    ...
    subgraph TEXT NOT NULL DEFAULT 'verify',  -- 'verify' or 'research'
...
WITH RECURSIVE traverse(id, depth) AS (
    ...
    WHERE source_node_id = ? 
      AND n.subgraph IN (?, ?)  -- restrict to allowed sub-graphs (e.g. 'verify')
    ...
    WHERE t.depth < ? 
      ...
      AND n.subgraph IN (?, ?)  -- block traversal into speculative sub-graphs
)
...
WITH RECURSIVE paths(path, last, depth) AS (
    ...
    WHERE source_node_id = ? 
      AND n.subgraph IN (?, ?)  -- restrict target to allowed sub-graphs
...
- Payload: `id`, `type`, `label`, `source_type`, `ontology_version`, `subgraph`

**Versioning:** Qdrant payloads store the `ontology_version` and include it as a filter in vector queries. This ensures that vector search results are consistent with the ontology version being queried, and handles vector embedding drift when node descriptions are edited.
...
- **Static data** (conductor, rank, torsion, isogeny class): **Indefinite TTL** — these are mathematical invariants that do not change. Cache until the ontology version changes.
- **Dynamic data** (search results, computed properties): 1h TTL
- Invalidation: on new ontology merge version

### 5.4 Future: Neo4j

When the node/edge count exceeds SQLite's performance ceiling (estimated: ~100,000+ nodes with proper indexing), migrate to Neo4j. The abstract `OntologyGraph` interface makes this a drop-in replacement:
...
- [ ] Set up SQLite database with schema and indexes
...
- [ ] Implement sub-graph partitioning (VERIFY / RESEARCH)
...
- [ ] Implement local cache (SQLite, indefinite TTL for static data)
...
- [ ] Implement conflict resolution (with LMFDB conflict logging)
- [ ] Implement versioning
- [ ] Write `version_query` function
- [ ] Implement Qdrant versioning (ontology_version in payload)
...
| G9 | Sub-graph isolation: VERIFY queries never return RESEARCH nodes |
| G10 | Qdrant versioning: vector search results are filtered by ontology_version |
| G11 | LMFDB cache: static data is cached indefinitely; dynamic data expires after 1h |
