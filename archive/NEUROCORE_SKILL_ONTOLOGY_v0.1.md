# Neurocore-Skill-Ontology — Design v0.1

**Status:** Final (incorporating independent review feedback)  
**Date:** 2026-07-08  
**Project root:** `/home/ubuntu/neurocore-skills/neurocore-skill-ontology/`  
**Related documents:**
  - [MASTER_ONTOLOGY_v0.1.md](../MASTER_ONTOLOGY_v0.1.md) — the Master Ontology this skill wraps
  - [ARX_ARCHITECTURE_v0.1.md](../ARX_ARCHITECTURE_v0.1.md) — Arx agent architecture (consumer, §3.9)
...
| S5 | **Fail gracefully.** | If the Master Ontology is unreachable, the skill returns a structured error. It never fabricates data. |
...
class CrossReferenceResult:
    ...
    fallback_rate: float # cumulative fallback rate for this session
...
| Error | HTTP Status | Skill Behavior |
|---|---|---|
| Ontology unreachable | Connection error | Return `OntologyUnavailableError` with message |
| Node not found | 404 | Return empty result (not an error) |
| Query timeout | 408 | Return `OntologyTimeoutError` |
| Invalid query | 400 | Return `OntologyQueryError` with details |
| Version not found | 404 | Return `OntologyVersionError` with available versions |

### 3.3 Caching

The skill implements agent-side caching for frequently accessed queries:

- Cache key: `{query_type}:{params_hash}:{version}`
- TTL: 5 minutes for lookups, 1 minute for searches, 30 seconds for traversals
- Cache invalidation: on explicit `version` parameter change, or after TTL expiry
- Cache store: in-memory (Python dict) — no Redis dependency for the skill itself
...
- [ ] Implement `ontology_bulk_cross_reference` with hash-keyed results
- [ ] Write unit tests
...
| G5 | Fallback rate is tracked and returned in cross-reference results |
| G6 | Error handling: ontology unreachable returns structured error, not crash |
| G7 | Caching: repeated identical queries return cached results within TTL |
| G8 | Arx integration: a proof audit with ontology cross-reference produces a complete provenance chain |
| G9 | Performance: lookup < 200ms (including network), search < 1s, cross-reference < 3s, bulk cross-reference < 5s for 10 claims |
