# Arx — Math Proof Audit Agent: Architecture Design v0.2.0

**Status:** Final (incorporating all independent review feedback)  
**Date:** 2026-07-08  
**Based on:** AXIOMA v1.9.1 (substrate + tool system + web UI), AXIOMA v2.0 (Cognition Layer), Skye meta-cognition (thought-addition v4), Neurogossip-agent-v3, NeuroCore + NeuroCore-Skills, J-Scope context management pattern, Master Ontology v0.2.0, Neurocore-Skill-Ontology v0.2.0  
...
| **45** | **Epistemic isolation is enforced.** | Independent review | Speculative concepts (consciousness measures, confidence < 0.8) are partitioned into a separate sub-graph. Verification queries are restricted to the `VERIFY` sub-graph (LMFDB, MathGLOSS, proof-theory concepts). |
| **46** | **Contradiction priority is category-based, not confidence-capped.** | Independent review | The priority of a contradiction (HIGH vs NORMAL) is determined by the category of the contradiction (structural vs. property-level), not by the source confidence. This prevents our ontology from being permanently capped below the escalation threshold. |
...
flowchart TB
    subgraph Arx["Arx Agent"]
        ...
        subgraph Meta["Meta-Cognition Layer (Skye-derived, v0.1: rule-based)"]
            ...
            FallbackRate["Fallback Rate Tracking<br/>(LLM fallback rate per audit)"]
        end
    ...
    subgraph Ontology["Ontology Layer"]
        MasterOnto["Master Ontology<br/>(SQLite + Qdrant for v0.2.0,<br/>Neo4j when needed)"]
...
| `proven-with-contradiction` | Step is formally proven but ontology contradicts the claim with HIGH priority | After formal proof + ontology cross-reference conflict |
| `proven-with-note` | Step is formally proven but ontology contradicts the claim with NORMAL priority | After formal proof + ontology cross-reference conflict |
...
Contradiction priority is determined by **category**, not by source confidence:

| Contradiction Category | Priority | Status | Description |
|---|---|---|---|
| **Structural** | HIGH | `proven-with-contradiction` | The claim asserts a structural property (e.g., "X is an elliptic curve") that contradicts ontology data. These are fundamental mismatches. |
| **Property-level** | NORMAL | `proven-with-note` | The claim asserts a numeric property (e.g., "X has rank 2") that contradicts ontology data. These are data-level mismatches. |
| **Mapping** | HIGH | `proven-with-contradiction` | The claim asserts a relationship (e.g., "X is isogenous to Y") that contradicts ontology data. These indicate mapping errors. |

This separation ensures that structural contradictions from our ontology (confidence 0.6) can still trigger HIGH priority escalation, because the priority is based on the *type* of contradiction, not the *source confidence*.
...
2. No NOEMA object is created (the contradiction is property-level, not structural)
...
Interface: Neurocore-Skill-Ontology (see NEUROCORE_SKILL_ONTOLOGY_v0.2.0.md)
Query types:
...
Status mapping:
  ...
  - Claim contradicted by ontology (structural): step receives status `proven-with-contradiction` if formally proven, else `provenance-broken`
  - Claim contradicted by ontology (property-level): step receives status `proven-with-note` if formally proven, else `gap-detected`
...
|---------|------------|----------------------------------|
| auditor | high | Step receives `ontology-unavailable`; audit continues but summary flags the gap. The step **cannot** receive `proven` without ontology cross-reference. |
...
If a step is already `proven` by a formal backend, ontology cross-reference is still run but its result is recorded as supplementary evidence. If a contradiction is found, the step receives `proven-with-contradiction` or `proven-with-note` (see §3.2.1).
...
| General mathematical expressions | N/A | **Undecidable** | By reduction from the word problem for groups (Novikov–Boone theorem, 1950s) |
...
- Fallback rate (LLM vs extractor) — tracked **per-audit**, not per-session
...
- Fallback rate (fraction of claims parsed by LLM fallback, per-audit)
...
A NeuroCore skill that wraps the Master Ontology API. See [NEUROCORE_SKILL_ONTOLOGY_v0.2.0.md](NEUROCORE_SKILL_ONTOLOGY_v0.2.0.md) for full design.
...
1. **LMFDB** — verified computational data on L-functions, modular forms, elliptic curves, number fields (confidence 1.0). The LMFDB contains **3,824,372** elliptic curves over ℚ (in 2,917,287 isogeny classes), **767,518** elliptic curves over number fields (in 367,933 isogeny classes), plus modular forms, L-functions, number fields, and other objects across dozens of categories. Total object count is in the low millions.
...
See [MASTER_ONTOLOGY_v0.2.0.md](MASTER_ONTOLOGY_v0.2.0.md) for the full design.

#### 3.9.1 Storage: SQLite + Qdrant for v0.2.0

For v0.2.0, the ontology is stored in **SQLite + Qdrant**, deferring Neo4j until we hit a performance ceiling.

**Rationale:**
- SQLite handles graph queries via recursive CTEs for the scale we expect in v0.2.0 (hundreds to low thousands of nodes)
- Properly indexed SQLite tables can handle hundreds of thousands of nodes/edges with recursive CTEs. The estimated performance ceiling is ~100,000+ nodes, not ~5,000 as initially estimated.
...
**Cache strategy:**
- **Static data** (conductor, rank, torsion, isogeny class): **Indefinite TTL** — these are mathematical invariants that do not change. Cache until the ontology version changes.
- **Dynamic data** (search results, computed properties): 1h TTL
- Cache stored in SQLite alongside the ontology
- Cache invalidation: on new ontology merge version
...
- Agents propose additions via a **sandboxed database table** or a dedicated queue, not by pushing directly to the Git repository
- A separate system tool or human operator handles git commits
- **Lark:** Approves ontology structure changes (new categories, new cross-source mappings)
- **Skye, Thea, Theoria, Axioma, Arx:** Can add individual concept additions within existing categories via the sandboxed queue
...
#### 3.9.5 Epistemic Isolation and Sub-graph Filtering
...
**Qdrant versioning:** Qdrant payloads store the `ontology_version` and include it as a filter in vector queries. This ensures that vector search results are consistent with the ontology version being queried.

#### 3.9.7 Re-Audit Trigger
...
1. A structured feedback record is generated:
   ```json
   {
     ...
     "contradiction_category": "structural | property | mapping",
...
3. Priority scheme:
   - **HIGH:** Structural contradictions and mapping contradictions (most urgent)
   - **NORMAL:** Property-level contradictions
...
#### 3.9.9 Ontology as Research Output
...
│   │   ├── sqlite_graph.py      # SQLite implementation (v0.2.0)
...
│   ├── ARX_ARCHITECTURE_v0.2.0.md
│   ├── MASTER_ONTOLOGY_v0.2.0.md
│   ├── NEUROCORE_SKILL_ONTOLOGY_v0.2.0.md
│   └── event_loop/
│       └── MULTI_AGENT_EVENT_LOOP_v0.2.0.md
└── README.md
```
...
R1: Assign status
    │  ├── Check all evidence
    │  ├── Apply contradiction detection (category-based priority)
...
Meta-Cognition: Generate quality report
    │  ├── Aggregate metrics
    │  ├── Compute fallback rate (per-audit)
...
Neurocore-Skill-Ontology
    │  ├── Validate query
    │  ├── Check server-side cache (rely on Master Ontology API cache)
    │  └── Forward to Master Ontology API
    │
    ▼
Master Ontology API
    │  ├── Parse query
    │  ├── Apply sub-graph filter (VERIFY vs RESEARCH)
    │  ├── Route to appropriate source adapter
    │  ├── LMFDB: on-demand query + cache (indefinite TTL for static data)
    │  ├── MathGLOSS: local graph query
    │  └── Our Ontology: local graph query (VERIFY sub-graph only for audits)
    │
    ▼
Merge results
    ...
    │  ├── Compute confidence
    │  ├── Attach provenance
    │  └── Log LMFDB conflicts even when LMFDB wins
    │
    ▼
Return to agent
```
...
- [ ] SQLite + Qdrant graph storage (abstract interface + SQLite implementation with proper indexes)
...
- [ ] LMFDB adapter (on-demand query + cache, indefinite TTL for static data)
...
- [ ] Sub-graph partitioning (VERIFY vs RESEARCH)
- [ ] Qdrant versioning (ontology_version in payload)
...
- A0.5e: Sub-graph filtering correctly blocks RESEARCH nodes during VERIFY queries
- A0.5f: SQLite recursive CTE traversal completes in < 100ms for 1000-node graph
...
- [ ] Fallback rate tracking (per-audit, not per-session)
...
- [ ] Contradiction detection with category-based priority (structural vs property-level vs mapping)
...
- A10: Contradiction between formal proof and ontology is detected and escalated with correct category-based priority
...
- [ ] Fallback rate surfaced in audit summary (per-audit)
...
3. **Neo4j vs SQLite threshold.** Properly indexed SQLite handles 100K+ nodes. We'll benchmark during Phase 0.5 to confirm the actual ceiling.
...
| Fallback rate | Fraction of claims parsed by LLM fallback vs hand-written extractors (tracked per-audit) |
...
- [MASTER_ONTOLOGY_v0.2.0.md](MASTER_ONTOLOGY_v0.2.0.md) — Master Ontology design
- [NEUROCORE_SKILL_ONTOLOGY_v0.2.0.md](NEUROCORE_SKILL_ONTOLOGY_v0.2.0.md) — Neurocore skill for ontology access
- LMFDB — https://www.lmfdb.org/ (3,824,372 elliptic curves over ℚ, 767,518 over number fields)
